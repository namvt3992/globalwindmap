document.addEventListener('DOMContentLoaded', function() {
    ...
    const googleSheetUrl = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTSwgVyafdXppewX8I7eVzmfAWRVzEQmQH8JH97YVuw1p7CeCMqMBsajbEdD68RaptqWSKG4UrYTMtI/pub?gid=0&single=true&output=csv';
    const map = L.map('map', { minZoom: 2, worldCopyJump: true, zoomControl: false }).setView([20, 0], 2);
    L.control.zoom({ position: 'topleft' }).addTo(map);

    const openTopoMap = L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png', { maxZoom: 17, attribution: '...' });
    const openStreetMap = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { maxZoom: 19, attribution: '...' });
    openTopoMap.addTo(map);
    const baseMaps = { "Topographic Map": openTopoMap, "Street Map": openStreetMap };
    
    // CẬP NHẬT: Chuyển bộ chọn bản đồ lên header
    ...
    // Dùng prepend để nó nằm ở đầu hàng ngang
    ...
    // CẬP NHẬT: Sửa lỗi lặp lại "All Countries"
    function populateCountryFilter(geojsonData) {
        countryChoice.clearStore(); // Xóa sạch các lựa chọn cũ
        ...
        countryChoice.setChoices(choicesArray, 'value', 'label', false); // false để không xóa placeholder
    }

    // CẬP NHẬT: Sửa lỗi lặp lại "All Projects"
    function updateProjectFilter() {
        projectChoice.clearStore(); // Xóa sạch các lựa chọn cũ
        ...
        projectChoice.setChoices(choicesArray, 'value', 'label', false); // false để không xóa placeholder
    }
    ...
    function applyFilters() { 
        ...
        const filteredFeatures = allGeoJsonData.features.filter(feature => {
            ...
            const statusMatch = selectedStatuses.includes(feature.properties.Status.trim());
            return countryMatch && projectMatch && statusMatch;
        });
        Object.values(layers).forEach(layer => map.removeLayer(layer));
        Object.keys(layers).forEach(status => layers[status].clearLayers());
        L.geoJSON(filteredFeatures, { pointToLayer: (feature, latlng) => { const status = feature.properties.Status.trim(); return L.circleMarker(latlng, { radius: 8, fillColor: statusColors[status] || '#6c757d', color: '#000', weight: 1.5, opacity: 1, fillOpacity: 0.8 }); }, onEachFeature: function(feature, layer) { const status = feature.properties.Status.trim(); if (layers[status]) { layers[status].addLayer(layer); } onEachFeature(feature, layer); } });
        Object.values(layers).forEach(layer => map.addLayer(layer));
        summaryBox.update(filteredFeatures, selectedCountry);
        if (selectedCountry === 'all' && selectedProject === 'all') { map.flyTo([20, 0], 2); }
        else if (countryData[selectedCountry]) { const data = countryData[selectedCountry]; map.flyTo([data.lat, data.lon], data.zoom); }
        else if (filteredFeatures.length > 0) { const [lon, lat] = filteredFeatures[0].geometry.coordinates; map.flyTo([lat, lon], 7); }
    }
    
    // --- CÁC HÀM CŨ (Không có thay đổi lớn) ---
    ...
    summaryBox.update = function(features = [], country = 'all') { 
        ...
        if (!features || features.length === 0) {
            ...
            const counts = features.reduce((acc, f) => { const status = f.properties.Status.trim(); acc[status] = (acc[status] || 0) + 1; return acc; }, {});
    ...
    function initializeDynamicStatusFilter(geojsonData) { const uniqueStatuses = [...new Set(geojsonData.features.map(f => f.properties.Status.trim()))].sort(); statusColors = {}; layers = {}; statusListContainer.innerHTML = ''; uniqueStatuses.forEach((status, index) => { const color = generateHslColor(index, uniqueStatuses.length); statusColors[status] = color; layers[status] = L.layerGroup(); const itemHTML = `<label class="status-item" style="--status-color: ${color};"><span class="status-dot" style="background-color: ${color};"></span><span class="status-name">${status}</span><input type="checkbox" value="${status}" checked><span class="slider"></span></label>`; statusListContainer.innerHTML += itemHTML; }); document.querySelectorAll('#status-list-container input[type="checkbox"]').forEach(checkbox => { checkbox.addEventListener('change', applyFilters); }); }
    function csvToGeoJSON(csv) { const lines = csv.split('\n'); const headers = lines[0].split(',').map(h => h.trim()); const features = []; for (let i = 1; i < lines.length; i++) { const currentline = lines[i].split(','); if (currentline.length < headers.length) continue; const properties = {}; headers.forEach((header, index) => { properties[header] = currentline[index] ? currentline[index].trim() : ''; }); if (properties.Latitude && properties.longitude && !isNaN(parseFloat(properties.Latitude)) && !isNaN(parseFloat(properties.longitude))) { features.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [parseFloat(properties.longitude), parseFloat(properties.Latitude)] }, properties: properties }); } } return { type: 'FeatureCollection', features: features }; }
    ...
    function displayInfo(properties) { document.getElementById('info-project-name').textContent = properties['Project Name'] || 'N/A'; document.getElementById('info-status').textContent = properties.Status || 'N/A'; document.getElementById('info-country').textContent = properties.Country || 'N/A'; document.getElementById('info-capacity').textContent = properties.Capacity ? `${properties.Capacity} MW` : 'N/A'; const detailsContainer = document.getElementById('info-details-container'); detailsContainer.innerHTML = ''; const excludedKeys = ['Project Name', 'Status', 'Country', 'Capacity', 'Latitude', 'longitude']; for (const key in properties) { if (!excludedKeys.includes(key) && properties[key] && properties[key].trim() !== '') { const itemDiv = document.createElement('div'); itemDiv.className = 'detail-item'; itemDiv.innerHTML = `<span class="detail-item-label">${key}</span><span class="detail-item-value">${properties[key]}</span>`; detailsContainer.appendChild(itemDiv); } } }
